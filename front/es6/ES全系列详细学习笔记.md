...
<script>
    ...
    let sum=add(1,2);
    console.log(sum);
    let add1=(a,b)=>a+b;//这个是更简洁的写法，但不具备通用性，不建议记住
    console.log(add1(2,3));
    //剩余参数，使用...声明，类似是一个数组
    //剩余参数，用于声明函数时候，如果不确定参数个数的时候，可以使用剩余参数
    let show=(a,b,...c)=>{
        console.log(a);
        console.log(b);
        console.log(c);
    ...
    show(1,2,3);
    //3最终会被放进一个数组里面，通过c打印出来
    show(1,2,3,4);
    //3，4最终会被放进一个数组里面，通过c打印出来
    //数组展开,通常用在连接数组上面，展开之后的内容不是具体的类型
    let arr=[1,2,3];
    console.log(arr);
    console.log(...arr);//此时...arr 变成了 1 2 3
    // let res=..arr;
    // let res='...arr';
    // console.log(res);
    let arr1=[4,5,6];
    let arr2=[...arr,...arr1];//可以使用...实现两个数组的连接
    console.log(arr2);
    //对象展开，使用...   声明新对象的时候，能够使用对象展开，把其他对象加进来
    let obj1={
        a:1,
        b:2
    };
    ...
    console.log(obj1);
    console.log(obj2);
    //字符串模板，可以在``（反单引号）里面使用${变量名},采用这种方式，可以避免拼接字符串过程中引号使用出错
    //"我来自辽宁省，年龄：28岁"
    let city='辽宁省';
    let age=28;
    let str="我来自"+city+"，年龄："+age+'岁';//学了字符串模板之后，这种写法少用
    console.log(str);
    let str1=`我来自${city}，年龄：${age}岁`;
    console.log(str1);
</script>
```
